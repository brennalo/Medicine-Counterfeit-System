...
function MapPicker({ onLocationSelect, selectedCoords }) {
    ...
    script.onload = () => {
      ...
      map.on("click", (e) => {
        ...
        fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json`)
          .then((r) => r.json())
          .then((d) => onLocationSelect({ lat, lng, address: d.display_name || `${lat.toFixed(6)}, ${lng.toFixed(6)}` }))
          .catch(() => onLocationSelect({ lat, lng, address: `${lat.toFixed(6)}, ${lng.toFixed(6)}` }));
      });
  ...
  return (
    <div>
      <div ref={mapRef} style={{ height: "300px", borderRadius: "12px", zIndex: 0 }} />
      <p className="text-xs text-slate-400 mt-2">📍 Click anywhere on the map to pin your location</p>
      {selectedCoords && (
...
export default function ManufacturerDashboard() {
  ...
  // ── Create Batch ──────────────────────────────────────────────────────────
  const [batchForm, setBatchForm] = useState({ medicineId: "", medicineName: "", hospitalId: "", expiryDate: "" });
  const [batchResult, setBatchResult] = useState(null);
  const [batchLoading, setBatchLoading] = useState(false);

  // ── Register Location ─────────────────────────────────────────────────────
  const [locForm, setLocForm] = useState({ name: "", locationType: "FACTORY", address: "", latitude: null, longitude: null });
  ...
  async function registerLocation(e) {
    ...
    try {
      ...
      if (res.ok) setLocForm({ name: "", locationType: "FACTORY", address: "", latitude: null, longitude: null });
    } finally {
      setLocLoading(false);
    }
  }
  ...
  async function updateBatchStatus() {
    ...
    if (!nextStatusCode) { alert("No further status update possible"); return; }
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <header className="border-b border-white/10 bg-white/5">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <button onClick={() => { document.cookie = "auth_token=; max-age=0"; window.location.href = "/"; }}
            className="text-sm text-slate-400 hover:text-white transition-colors">Sign out</button>
        </div>
      </header>

      <main className="max-w-5xl mx-auto px-6 py-8">
        <div className="flex gap-2 mb-8 flex-wrap">
          {[{ key: "batches", label: "Create Batch" }, { key: "locations", label: "Register Location" }, { key: "update", label: "Update Batch Status" }].map((tab) => (
        ...
        {activeTab === "batches" && (
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 max-w-lg">
            ...
            <p className="text-sm text-slate-400 mb-6">Batch ID is generated via keccak256 hash and stored on-chain.</p>
            {batchResult && (
              <div className={`rounded-xl p-4 text-sm mb-4 ${batchResult.ok ? "bg-green-500/20 border border-green-500/30 text-green-300" : "bg-red-500/20 border border-red-500/30 text-red-300"}`}>
                {batchResult.ok ? (
                  <div>
                    ...
                    <div className="font-mono text-xs mt-1 break-all">ID: {batchResult.batchId}</div>
                    <div className="text-xs mt-1 opacity-60">tx: {batchResult.txHash}</div>
                  </div>
                ) : `❌ ${batchResult.error}`}
              </div>
            )}
            <form onSubmit={createBatch} className="space-y-4">
              {[["Medicine ID", "medicineId", "e.g. MED-12345"], ["Medicine Name", "medicineName", "e.g. Paracetamol 500mg"], ["Hospital ID", "hospitalId", "Destination hospital user ID"]].map(([label, key, ph]) => (
                <div key={key}>
                  <label className="block text-sm text-slate-300 mb-1">{label}</label>
                  <input type="text" value={batchForm[key]} onChange={(e) => setBatchForm({ ...batchForm, [key]: e.target.value })}
              ...
              <div>
                ...
                <input type="date" value={batchForm.expiryDate} onChange={(e) => setBatchForm({ ...batchForm, expiryDate: e.target.value })}
                  className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-violet-400" required />
              </div>
        ...
        {activeTab === "locations" && (
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 max-w-2xl">
            ...
            <p className="text-sm text-slate-400 mb-6">
              Pin your location on the map — coordinates and address are extracted automatically.
            </p>
            {locResult && (
              <div className={`rounded-xl p-4 text-sm mb-4 ${locResult.ok ? "bg-green-500/20 border border-green-500/30 text-green-300" : "bg-red-500/20 border border-red-500/30 text-red-300"}`}>
                {locResult.ok ? (
                  <div>
                    ...
                    <div className="text-xs mt-1">ID: <span className="font-mono">{locResult.locationId}</span></div>
                    <div className="font-mono text-xs mt-1 break-all opacity-60">Hash: {locResult.locationDataHash}</div>
                  </div>
                ) : `❌ ${locResult.error}`}
              </div>
            )}
            <form onSubmit={registerLocation} className="space-y-5">
              ...
              <div>
                <label className="block text-sm text-slate-300 mb-2">Pin Location on Map</label>
                <MapPicker
                  onLocationSelect={handleMapSelect}
                  selectedCoords={locForm.latitude ? { lat: locForm.latitude, lng: locForm.longitude } : null}
                />
              </div>

              {locForm.address && (
                <div>
                  <label className="block text-sm text-slate-300 mb-1">Address (auto-detected)</label>
              ...
              <button type="submit" disabled={locLoading || !locForm.latitude}
                className="w-full bg-violet-500 hover:bg-violet-400 disabled:opacity-50 text-white font-medium rounded-lg py-2.5 transition-colors">
                {locLoading ? "Registering on-chain…" : !locForm.latitude ? "📍 Pin a location on the map first" : "Register Location"}
              </button>
            </form>
          </div>
        )}
        ...
        {activeTab === "update" && (
          <div className="max-w-lg">
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-4">
              <div>
                <h2 className="text-lg font-semibold mb-1">Update Batch Status</h2>
                <p className="text-sm text-slate-400">
                  Your GPS is detected automatically and matched against your registered locations. Expiry is checked automatically.
                </p>
              </div>

              <div className="flex gap-3">
                <input type="text" value={updateBatchId}
                  onChange={(e) => { setUpdateBatchId(e.target.value); setBatchInfo(null); setUpdateResult(null); }}
              ...
              {batchInfo && !batchInfo.error && (
                <div className="bg-white/5 rounded-xl p-4 space-y-4">
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      <span className="text-slate-400 text-xs block">Medicine</span>
                      <span className="font-medium">{batchInfo.medicineName}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 text-xs block">Expiry Date</span>
                      <span className="font-medium">{new Date(batchInfo.expiryDate).toLocaleDateString()}</span>
                    </div>
                  </div>
                  ...
                  {NEXT_STATUS[batchInfo.currentStatus] ? (
                    <>
                      <div className="text-xs bg-violet-500/10 border border-violet-500/20 rounded-lg px-3 py-2 space-y-1 text-slate-300">
                        <div>🔜 Next: <strong className="text-white">{STATUS_LABELS[NEXT_STATUS[batchInfo.currentStatus]]}</strong></div>
                        <div className="text-slate-400">📍 GPS will be matched against your registered locations automatically</div>
                        <div className="text-slate-400">📅 Expiry date will be checked automatically</div>
                      </div>

                      <div>
                        <label className="block text-sm text-slate-300 mb-1">
                          Image Proof <span className="text-slate-500 text-xs">(optional)</span>
                        </label>
                      ...
                      <button onClick={updateBatchStatus} disabled={updateLoading}
                        ...
                        {updateLoading ? "📍 Detecting location & submitting…" : `Update to ${STATUS_LABELS[NEXT_STATUS[batchInfo.currentStatus]]}`}
                      </button>
                    </>
                  ) : (
                    <div className="text-sm text-slate-400 text-center py-2">
                      {batchInfo.currentStatus === "DELIVERED" ? "⏳ Awaiting hospital verification" : "✅ No further updates possible"}
                    </div>
                  )}
                </div>
              )}
                  ...
                  {updateResult.ok ? (
                    <div className="space-y-1">
                      {updateResult.flagged ? (
                        <>
                          <div>⚠️ Stored but <strong>FLAGGED</strong></div>
                          <div className="text-xs">Reason: {updateResult.flagReasonLabel}</div>
                        </>
                      ) : (
                        <>
                          <div>✅ Status updated on-chain</div>
                          <div className="text-xs">Matched location: <span className="font-mono">{updateResult.matchedLocationId}</span></div>
                        </>
                      )}
                      <div className="text-xs opacity-60">tx: {updateResult.txHash?.slice(0, 24)}…</div>
                    </div>
                  ) : `❌ ${updateResult.error}`}
                </div>
              )}
